...
void GamePath::Draw() {
    ...
    for(int i=0;i<mSmoothLength;i++)
    {
        ...
        if(mSmoothPathDrawZ[i] == 0)Graphics::SetColor(1.0f, 0.0f, 0.5f);
        if(mSmoothPathDrawZ[i] == 1)Graphics::SetColor(0.0f, 1.0f, 0.5f);
        if(mSmoothPathDrawZ[i] == 2)Graphics::SetColor(0.15f, 0.25f, 1.0f);
        
        Graphics::DrawRect(aX - 2, aY - 2, 5.0f, 5.0f);
    }
