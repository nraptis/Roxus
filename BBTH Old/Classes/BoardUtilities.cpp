...
BoardUtilities::BoardUtilities(Board *pBoard)
{
    mBoard = pBoard;
}
...
void BoardUtilities::DrawDirt(int *pDirtTop, int pColumnCount, int pStartY, int pEndY)
{
    ...
    float aTileWidth = gTileWidth;
    float aTileHeight = gTileHeight;
    ...
    Graphics::MatrixPush();
    Graphics::Scale(gTileWidth / 64.0f);
...
void BoardUtilities::DrawDirtSide(int pGridX, int pGridY, bool pRight, bool pTop)
{
    ...
    if(pTop)aYStart = aImageCenterY - aTileHeight / 2.0f;
...
void BoardUtilities::DrawDirtCorner(int pGridX, int pGridY, bool pRight)
{
    ...
    Graphics::SetColor(gRand.F(), gRand.F(), gRand.F());
...
bool BoardUtilities::TileRendersAsDirt(int pGridX, int pGridY)
{
    ...
    if((pGridX >= 0) && (pGridX < mBoard->mGridWidth) && (pGridY >= 0) && (pGridY < mBoard->mGridHeight))
    {
        if(mBoard->mDirtTop[pGridX] <= pGridY)
        {
            aReturn = true;
        }
    }
...
FParticle *BoardUtilities::GenerateSparkle()
{
    return GenerateSparkle(&(gApp->mEffectParticleWarm [gRand.Get(4)]));
}

FParticle *BoardUtilities::GenerateSparkle(FSprite *pSprite)
{
...
FParticle *BoardUtilities::GenerateTwinkle()
{
    return GenerateTwinkle(&(gApp->mEffectTwinkleWhite[0]), 0.4f + gRand.GetFloat(0.2f));
}

FParticle *BoardUtilities::GenerateTwinkle(FSprite *pSprite, float pSpeed)
{
    FParticle *aReturn = new FParticle(pSprite);
