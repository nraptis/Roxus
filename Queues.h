...
public:
	...
	void * Dequeue()
	{
		...
		if(aLast != mHeapRoot)aLast=aLast->mHeapParent->mHeapLeft;
		while (aLast->mHeapRight != 0)aLast=aLast->mHeapRight;


		//Remove Pointer To Last Item!
        if(mHeapLast->mHeapParent->mHeapLeft == mHeapLast)mHeapLast->mHeapParent->mHeapLeft = 0;
        else mHeapLast->mHeapParent->mHeapRight = 0;
