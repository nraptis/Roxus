...
void VertexBuffer::Add(float x, float y, float z, float u, float v, float w)
{
	//printf("VBUFFER %f %f\n", u, v);
...
void VertexBuffer::AddUVW(float u, float v, float w)
{
...
void VertexBuffer::AddXYZ(float x, float y, float z)
{    
...
void VertexBuffer::AddQuad(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4)
{
...
{
    SetColor(1,0,0,0.5f);
    DrawRect(x1-2,y1-2,5,5);
    
    SetColor(0,1,0,0.5f);
    DrawRect(x2-2,y2-2,5,5);
    
    SetColor(0,0,1,0.5f);
    DrawRect(x3-2,y3-2,5,5);
    
    SetColor(1,1,1,0.5f);
    DrawRect(x4-2,y4-2,5,5);
...
void VertexBuffer::AddRGBA(float pRed, float pGreen, float pBlue, float pAlpha)
{
...
void VertexBuffer::StereoscopicShift(float pShiftMin, float pShiftMax, VertexBuffer *pBuffer, Color pColorOuter, Color pColorInner)
{
    mCoordCount=0;
    mVertexCount=0;
    mColorCount=0;
...
void VertexBuffer::DrawColored()
{
    /*
    if(mVertexCount>0)
	{
		glVertexPointer(3,GL_FLOAT,0,mXYZ);
		glTexCoordPointer(2,GL_FLOAT,0,mUVW);
...
void VertexBuffer::Draw(int pCount)
{
    Draw(0, pCount);
}

void VertexBuffer::Draw(int pStart, int pCount)
{
    ...
    if(mBindIndex != -1)
    {
        gGraphics.EnableTexture();
        gGraphics.BindTexture(mBindIndex);
    }
...
void VertexBuffer::DrawStrips()
{
    DrawStrips(mVertexCount);
}

void VertexBuffer::DrawStrips(int pCount)
{
    DrawStrips(0, pCount);
}

void VertexBuffer::DrawStrips(int pStart, int pCount)
{
	//if(pStart<0)pStart=0;
    //if(pCount>mVertexCount)pCount=mVertexCount;
    ...
    //Graphics::DrawTriangleStrips(pCount,&mXYZ[pStart*3],&mUVW[pStart*3]);
}

void VertexBuffer::GenerateMagicCube(float pSize)
{
...
void VertexBuffer::GeneratePanoCube(float pSize)
{
	//Front Face
...
void VertexBuffer::GenerateCube(float aSize)
{
