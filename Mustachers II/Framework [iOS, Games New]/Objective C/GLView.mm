...
- (UIImage*)drawToImage:(unsigned char*)targetBuffer withFlipBuffer:(unsigned char*)targetBufferFlipped withWidth:(int)targetWidth withHeight:(int)targetHeight
{
    ...
    NSInteger myDataLength = gAppWidth * gAppHeight * 4;
	
    glReadPixels(0, 0, gAppWidth, gAppHeight, GL_RGBA, GL_UNSIGNED_BYTE, targetBuffer);
    
    //int aHeight4 = gAppWidth * 4;
    int aWidth4 = gAppWidth * 4;
    
    for(int y = 0; y < gAppHeight; y++)
    {
...
- (UIImage*)convertToImage
{
	...
	glOrthof(0, gAppWidth, gAppHeight, 0, -100, 100);
	gApp->BaseDraw();
