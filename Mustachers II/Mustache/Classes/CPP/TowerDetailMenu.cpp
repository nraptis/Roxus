...
void TowerDetailMenu::Draw()
{
    if(mHidden == false)
    {
        SetColor(0.15f, 1.0f, 0.15f, 0.4f);
        
        float aWidth = mWidth * mScale;
        float aHeight = mHeight * mScale;
        
        DrawRect(mCenterX - aWidth / 2.0f, mCenterY - aHeight / 2.0f, aWidth, aHeight);
        
        SetColor(1.0f);
        
        if(mButtonClickedIndex == 0)SetColor(0.75f,0.75f,0.75f);else SetColor();
        mApp->mTowerDetailMenuButtonInfo.Draw(mButtonX[0], mButtonY[0], mScale, 0.0f);
        
        if(mButtonClickedIndex == 1)SetColor(0.75f,0.75f,0.75f);else SetColor();
        mApp->mTowerDetailMenuButtonSell.Draw(mButtonX[1], mButtonY[1], mScale, 0.0f);
        
        if(mButtonClickedIndex == 2)SetColor(0.75f,0.75f,0.75f);else SetColor();
