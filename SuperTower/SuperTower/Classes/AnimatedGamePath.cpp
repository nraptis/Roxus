...
void AnimatedGamePath::Update() {
    ...
    mTextureAnimationOffset += 0.0035f;
...
void AnimatedGamePath::GenerateTextureQuads() {
    ...
    EnumList(AnimatedGamePathChunk, aChunk, mPathChunkList) {
        ...
        aChunk->mDrawNodeList.Reset();
        aChunk->mBuffer.Reset();


        float aR = 1.0f;
        float aG = 1.0f;//gRand.F(0.85f, 1.0f);
        float aB = 1.0f;//gRand.F(0.85f, 1.0f);
        float aA = 1.0f;//gRand.F(0.85f, 1.0f);


        AnimatedGamePathNode *aPrevNode = (AnimatedGamePathNode *)aChunk->mPathNodeList.mData[0];
        float aPrevX1 = aPrevNode->mX1;
        float aPrevY1 = aPrevNode->mY1;
        float aPrevX2 = aPrevNode->mX2;
        float aPrevY2 = aPrevNode->mY2;
        ...
        for (int i=1;i<aChunk->mPathNodeList.mCount;i++) {
            ...
            aChunk->mBuffer.Add(aPrevX1, aPrevY1 - 100.0f, aPrevU, 0.0f);
            aChunk->mBuffer.Add(aPrevX2, aPrevY2 - 100.0f, aPrevU, 1.0f);
            aChunk->mBuffer.Add(aX1, aY1 - 100.0f, aU, 0.0f);
