...
void FPointPath::Clear()
{
    FPointList::Clear();
    mRefresh = true;
}

void FPointPath::Reset()
{
    FPointList::Reset();
    mRefresh = true;
}

void FPointPath::Add(FPointList *pList)
{
    FPointList::Add(pList);
    mRefresh = true;
}

void FPointPath::Add(float pX, float pY)
{
    FPointList::Add(pX, pY);
    mRefresh = true;
}

void FPointPath::Set(int pIndex, float pX, float pY)
{
    FPointList::Set(pIndex, pX, pY);
    mRefresh = true;
}
...
/*
void FPointPath::Get(float pPercent, float &pX, float &pY)
{
    if(mRefresh)
    {
        Generate();
    ...
    if(mCount > 1)
    {
        ...
        float aGetLen = pPercent * mLength;
        if(aGetLen >= mLength)aGetLen = mLength;
        
        aInterpP = mInterp.Get(aGetLen, mLength, aInterpI1, aInterpI2);
        
        pX = mX[aInterpI1] + (mX[aInterpI2] - mX[aInterpI1]) * aInterpP;
        pY = mY[aInterpI1] + (mY[aInterpI2] - mY[aInterpI1]) * aInterpP;
    }
    else if(mCount == 1)
    {
        pX = mX[0];
...
void FPointPath::GetWithDist(float pDistance, float &pX, float &pY)
{
    if(mRefresh)
    {
        Generate();
    ...
    if(mCount > 1)
    {
        ...
        aInterpP = mInterp.Get(pDistance, mLength, aInterpI1, aInterpI2);
...
void FPointPath::GetWithPercent(float pPercent, float &pX, float &pY)
{
    GetWithDist(pPercent * mLength, pX, pY);
}

void FPointPath::GetRandom(float &pX, float &pY)
{
    GetWithDist(gRand.GetFloat(mLength), pX, pY);
}

void FPointPath::Generate()
{
    mLength = 0;
