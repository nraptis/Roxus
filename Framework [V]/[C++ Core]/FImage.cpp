            ...
            for(int aSuffixIndex = 0; (aSuffixIndex < gAppBase->mImageLoadSuffixList.mCount) && (aDidLoad == false); aSuffixIndex++)
            {
                ...
                for(int aMutableSuffixIndex = 0; (aMutableSuffixIndex < gAppBase->mImageLoadMutableSuffixList.mCount) && (aDidLoad == false); aMutableSuffixIndex++)
                {
                    ...
                    if(aDidLoad)
                    {
                        ...
                        aWriteIndex = cImageLoadBufferAlphaMask.Write("_alpha", aWriteIndex);
                        aWriteIndex = cImageLoadBufferAlphaMask.Write(aPathMutableSuffix->c(), aWriteIndex, aPathMutableSuffix->mLength);
                        aWriteIndex = cImageLoadBufferAlphaMask.Write((const char *)".", aWriteIndex, 1);
                        
                        FImage aMask;
                        
                        for(int aExtensionIndex = (gAppBase->mImageLoadExtensionList.mCount - 1); (aExtensionIndex >= 0) && (aDidLoadMask == false); aExtensionIndex--)
                        {
                            aPathExtension = (FString *)(gAppBase->mImageLoadExtensionList.Fetch(aExtensionIndex));
...
/*
 ...
 mScale=1.0f;
	...
	if(mExpandedWidth==0||mExpandedHeight==0)
	{
 //Log("Must have had an error loading: [%s]\n", aPath.c());
	}
 }
...
/*
 ...
 Graphics::TextureSetData(mBindIndex,mData,mExpandedWidth,mExpandedHeight);
 ...
 void FImage::Bind()
 {
	...
	{
 //Log("Can't Bind: [W:%d H:%d I:%d]\n", mExpandedWidth, mExpandedHeight, mBindIndex);
 return;
	}
	
	MakePowerOf2();
 
 mBindIndex = Graphics::TextureGenerate(mData, mExpandedWidth, mExpandedHeight);
 //mBindIndex = Graphics::GenerateTexture(mData, mExpandedWidth, mExpandedHeight);
